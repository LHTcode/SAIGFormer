...
def read_img_smid(path):
    """read image by cv2 or from lmdb
    return: Numpy float32, HWC, BGR, [0,1]"""
    img = np.load(path)
    assert img is not None, ValueError('Image not found: {}'.format(path))
...
def read_img_seq_smid(path):
    ...
    img_l = [read_img_smid(v) for v in img_path_l]
    # stack to Torch tensor
    imgs = np.stack(img_l, axis=0)
    try:
        imgs = imgs[:, :, :, [2, 1, 0]]
...
def read_img_seq_sid(path):
    ...
    img_l = [read_img_sid(v) for v in img_path_l]
    # stack to Torch tensor
    imgs = np.stack(img_l, axis=0)
    try:
        imgs = imgs[:, :, :, [2, 1, 0]]
...
def smid_img_to_tensor(imgs: list):
    """Convert a sequence of images to a tensor
    Args:
        imgs (np.ndarray): size (T, H, W, C), RGB, [0, 1]

    Returns:
        imgs_tensor (Tensor): size (T, C, H, W), RGB, [0, 1]
    """
    def _totensor(img):
...
def sid_img_to_tensor(imgs: list):
    """Convert a sequence of images to a tensor
    Args:
        imgs (np.ndarray): size (T, H, W, C), RGB, [0, 1]
