...
def parse_options(is_train=True):
    ...
    parser.add_argument(
        '--opt', type=str, default='Options/SAIGFormer_lol_blur.yml', help='Path to option YAML file.')
    parser.add_argument(
        '--launcher',
...
def init_loggers(opt):
    ...
    if opt['logger'].get('use_tb_logger') and 'debug' not in opt['name']:
        logger_path = osp.join(opt['logger'].get('logger_path'), opt['name'])
        tb_logger = init_tb_logger(log_dir=logger_path)
