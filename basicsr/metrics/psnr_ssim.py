...
def calculate_psnr(img1,
    ...
    if type(img1) == torch.Tensor:
        ...
        img1 = img1.detach().cpu().numpy().transpose(0,2,3,1)   # 'HWC'
    if type(img2) == torch.Tensor:
        if len(img2.shape) == 3:
            img2 = img2.unsqueeze(0)
        img2 = img2.detach().cpu().numpy().transpose(0,2,3,1)   # 'HWC'
    ...
    if (mse == 0).any():
        return float('inf')
    max_value = np.ones_like(mse) if (img1.max(axis=(1,2,3)) <= 1).all() else np.ones_like(mse)*255.
    return (20. * np.log10(max_value / np.sqrt(mse))).mean()
