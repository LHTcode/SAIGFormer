...
class Mixing_Augment:
    ...
    def mixup(self, target, input_):
        ...
        while torch.all(r_index == torch.arange(target.size(0), device=self.device)):
            r_index = torch.randperm(target.size(0)).to(self.device)
    ...
    def __call__(self, target, input_):
        if self.use_identity:
            ...
            if augment < len(self.augments):        # P = 50%
...
class ImageCleanModel(BaseModel):
    ...
    def init_training_settings(self):
        ...
        if train_opt.get('pixel_opt'):
            ...
            for loss_conf in losses:
                ...
                self.cri_pix.append(cri_pix_cls(**loss_conf).to(
        ...
        # SAM optimizer's area
        train_opt = self.opt['train']
        optim_type = train_opt['optim_g'].pop('type', None)
        self.sam_optim = False
        if optim_type is not None and optim_type == 'SAM':
            self.optimizer_g = optimizer.SAM(self.optimizer_g.param_groups, self.optimizer_g, **train_opt['optim_g'])
    ...
    def setup_optimizers(self):
        ...
        if optim_type == 'Adam':
            ...
            base_optimizer_type = train_opt['optim_g']['base_optimizer'].pop('type')
            base_optimizer_config = train_opt['optim_g'].pop('base_optimizer')
            if base_optimizer_type == 'Adam':
                self.optimizer_g = torch.optim.Adam(
                    optim_params, **base_optimizer_config)
            elif base_optimizer_type == 'AdamW':
                self.optimizer_g = torch.optim.AdamW(
                    optim_params, **base_optimizer_config)
            elif base_optimizer_type == 'SGD':
                self.optimizer_g = torch.optim.SGD(
                    optim_params, **base_optimizer_config)
            else: 
                raise NotImplementedError(
                    f'base optimizer {base_optimizer_type} is not supperted yet.')
        else:
            raise NotImplementedError(
    ...
    def optimize_parameters(self, current_iter):
        ...
        with autocast(device_type='cuda', enabled=self.use_amp):
            ...
            for pred in preds:
                ...
                for loss in self.cri_pix:
                    l_pix += loss(pred, self.gt) #此处统计batch的loss
        ...
        if self.sam_optim:
            def closure(losses, model, lq, gt, amp_scaler):
                pred = torch.clamp(model(lq), 0, 1)
                l_pix = 0.
                for loss in losses:
                    l_pix += loss(pred, gt)
                amp_scaler.scale(l_pix).backward()
                return l_pix
            self.amp_scaler.step(self.optimizer_g, closure, self.cri_pix, self.net_g, self.lq, self.gt, self.amp_scaler)
        else:
    ...
    def pad_test(self, window_size):
        ...
        self.output = torch.clamp(self.output[:, :, 0:h -
                                  mod_pad_h * scale, 0:w - mod_pad_w * scale], 0, 1)
