...
parser.add_argument(
    '--opt', type=str, default='Options/SAIGFormer_lolv1.yml', help='Path to option YAML file.')
parser.add_argument('--weights', default='pretrained_weights/lolv1_pretrained_weight.pth',
...
psnr = []
ssim = []
if dataset in ['SID', 'SMID', 'LOL_Blur']:
    ...
    if dataset == 'SID':
        ...
        from basicsr.data.LOL_Blur_dataset import Dataset_LOLBlurImage as Dataset
    else:
    ...
    with torch.inference_mode():
        for inp_path, tar_path in tqdm(zip(input_paths, target_paths), total=len(target_paths)):
            ...
            print('\n', f'id: {os.path.splitext(os.path.split(inp_path)[-1])[0]}', f'PSNR: {utils.PSNR(target, restored)}', f'SSIM: {utils.calculate_ssim(img_as_ubyte(target), img_as_ubyte(restored))}')
